...
def YUV2RGB(yuv):
    [Y,U,V] = yuv
    R = min(max(round(Y+1.4075*(V-128)), 0), 255)
    G = min(max(round(Y-0.3455*(U-128)-(0.7169*(V-128))), 0), 255)
    B = min(max(round(Y+1.7790*(U-128)), 0), 255)
...
def green_filter(im, param):
    ...
    color_green = np.array([140,60,75]) # centre of used filter
    filter_width= np.array([255,0,0]) * param + np.array([0,60,75])
    ...
    mask = np.all(im_pixels > YUV2RGB(color_green - filter_width), axis=1) * np.all(im_pixels < YUV2RGB(color_green + filter_width),
                                                                            axis=1)
    ...
    filtered_im.show()
    # print(f"filter_width_yuv = {filter_width_yuv}")
    # print(f"filter_width = {filter_width}")
    # print(f"color_green_yuv = {green_yuv}")
    # print(f"color_green_rgb = {color_green}")
    return filtered_im
...
def generate_ROC_plot():
    ...
    for param in np.linspace(0.0, 0.3, 20):

        print(f"filter_width_yuv = {np.array([255,0,0]) * param}")
        # Initialize totals
        true_positives = 0
        false_positives = 0
